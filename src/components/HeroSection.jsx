...
export default function HeroSection({ t }) {
  ...
  return (
    <section id="inicio" className="relative min-h-[66.666vh] overflow-hidden pt-20 lg:h-[66.666vh]">
      <img
      ...
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-4 px-4 pb-4 pt-2 sm:gap-6 sm:px-6 sm:pb-6 sm:pt-3 lg:h-full lg:grid-cols-[1.05fr_0.95fr] lg:gap-2 lg:px-8">
        <motion.div
          initial={{ opacity: 0, x: -32 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.65, ease: "easeOut" }}
          className="z-10 max-w-lg lg:self-start lg:pt-2"
        >
          <p className="mb-1 font-body text-xs font-semibold uppercase tracking-[0.24em] text-brand-primary sm:text-sm">
            EatBurger
          </p>
          <h1 className="font-heading text-[2.05rem] leading-[0.93] text-white sm:text-[2.55rem] lg:text-[3.3rem] xl:text-[3.9rem]">
          ...
          <p className="mt-2 max-w-lg font-body text-sm leading-snug text-brand-muted sm:text-base lg:text-[0.98rem]">
            {t.hero.description}
          </p>
